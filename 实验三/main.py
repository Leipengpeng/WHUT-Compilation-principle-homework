...
def scoreCount(mA,mB,setList,tryList):
    ...
    for i in range(lenth):
        ...
        tempSetList.append({'class': 'N','element': tryList[i]["first"]})
        tempSetList.append({'class': 'N','element': tryList[i]["second"]})
        #计算
        tempScore=0
    ...
    while setNum>finishedNum:
        ...
        if len(temp['element'])>1:
            ...
            if isIn(temp1,setList)==False or isIn(temp2,setList)==False:
                ...
                #加入新的元素
                setList.append({"class": 'N',  "element": out1})
                setList.append({"class": 'N',  "element": out2})
                setNum=len(setList)
                finishedNum=countFinish(mA,mB,setList)
...
if __name__ == '__main__':
    ...
    print('函数计算结束，下面开始分组输出结果')
    out=list(out)
    for i in range(len(out)):
        print('串组:   '+str(out[i]['element'])+'类型:   '+str(out[i]['class']))
